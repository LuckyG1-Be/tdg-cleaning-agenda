...
export default function CustomersClient() {
  ...
  async function createNew() {
    ...
    setError(null);
    try {
      const r = await fetch("/api/customers", { method: "POST", headers: {"content-type":"application/json"}, body: JSON.stringify({}) });
      const j = await r.json();
  ...
  async function save() {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/customers/${selected.id}`, { method: "PATCH", headers: {"content-type":"application/json"}, body: JSON.stringify(selected) });
      const j = await r.json();
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-1 bg-white rounded-2xl border border-zinc-200 shadow-sm p-4">
        <div className="flex items-center justify-between gap-2 mb-3">
          <input className="w-full rounded-xl border border-zinc-200 px-3 py-2"
            placeholder="Zoek (naam, bedrijf, stad, email, tel)"
            value={q} onChange={(e)=>setQ(e.target.value)} />
          <button disabled={busy} onClick={createNew} className="shrink-0 rounded-xl bg-zinc-900 text-white px-3 py-2 font-medium hover:bg-zinc-800 disabled:opacity-60">
            + Nieuw
          </button>
        </div>
        ...
        <div className="divide-y divide-zinc-100 max-h-[60vh] overflow-auto">
          {items.map((c) => (
            ...
            >
              ...
              <div className="text-xs text-zinc-500">{[c.street, c.number, c.city].filter(Boolean).join(" ")}</div>
            </button>
          ))}
          {!items.length ? <div className="text-sm text-zinc-500 py-6 text-center">Geen klanten</div> : null}
        </div>
      </div>

      <div className="lg:col-span-2 bg-white rounded-2xl border border-zinc-200 shadow-sm p-4">
        {!selected ? (
          <div className="text-sm text-zinc-600">Selecteer een klant links, of klik op <span className="font-medium">+ Nieuw</span>.</div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2">
                <button disabled={busy} onClick={save} className="rounded-xl bg-zinc-900 text-white px-4 py-2 font-medium hover:bg-zinc-800 disabled:opacity-60">
                  Opslaan
                </button>
                <button disabled={busy} onClick={del} className="rounded-xl border border-zinc-200 px-4 py-2 font-medium hover:bg-zinc-50 disabled:opacity-60">
                  Verwijderen
                </button>
              </div>
            </div>
              ...
              {[
                ...
                <div key={k}>
                  ...
                  <input className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2"
                    value={(selected as any)[k] || ""} onChange={(e)=>setField(k, e.target.value || null)} />
                </div>
              ))}
            </div>
            ...
            <div>
              ...
              <textarea className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 min-h-[140px]"
                value={selected.notes || ""} onChange={(e)=>setField("notes", e.target.value || null)} />
            </div>
